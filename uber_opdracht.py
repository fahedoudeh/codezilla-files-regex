...
#   - Toon de lijst met Uber-diensten
    for key, service in UBER_SERVICES.items():
        print(f"{key}. {service["name"]}")
#   - Vraag de gebruiker om een geldige keuze (1, 2 of 3)
    choice = input("Voer het nummer van uw keuze in: ").strip()
#   - Valideer invoer met een while-loop
    while choice not in UBER_SERVICES:
        choice = input("Ongeldige keuze. Probeer opnieuw: ").strip()
    return choice    
...
#   - Gebruik try-except voor invoerfouten
    while True:
        try:
            distance = float(input("Voer het aantal kilometers van uw rit in: "))
            if distance > 0:
                return distance
            else:
                print("Voer een positief getall.")
        except ValueError:
            print("Ongeldig invoer. Gebruik een getal.")
